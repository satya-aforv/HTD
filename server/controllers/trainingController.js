...
export const getAllTrainings = async (req, res) => {
  try {
    ...
    if (technology) query['modules.technology'] = { $regex: technology, $options: 'i' };
    if (search) {
      query.$or = [
...
export const createTraining = async (req, res) => {
  try {
    ...
    const existingTraining = await Training.findOne({ trainingId });
    if (existingTraining) {
      return res.status(400).json({ message: 'Training with this ID already exists' });
    }
    
    // Generate training ID if not provided
    const generatedTrainingId = trainingId || `TRN-${Date.now().toString().slice(-6)}`;
    ...
    await Candidate.findByIdAndUpdate(candidate, { status: 'IN_TRAINING' });
...
export const updateTraining = async (req, res) => {
  try {
    ...
    if (updateData.status === 'COMPLETED') {
      await Candidate.findByIdAndUpdate(training.candidate, { status: 'DEPLOYED' });
    }
