// server/seeders/simpleHospitalSeeder.js
...
// Hospital permissions data
const hospitalPermissions = [
  { name: 'View Hospitals', description: 'Can view hospitals list and details', resource: 'hospitals', action: 'view' },
  { name: 'Create Hospitals', description: 'Can create new hospitals', resource: 'hospitals', action: 'create' },
  { name: 'Update Hospitals', description: 'Can update existing hospitals', resource: 'hospitals', action: 'update' },
...
async function seedHospitalPermissions() {
  console.log('🏥 Starting Hospital Permissions Seeder...');
  console.log('=====================================');
  ...
  try {
    ...
    // Check if hospital permissions already exist
    console.log('🔍 Checking for existing hospital permissions...');
    const existingPermissions = await Permission.find({ resource: 'hospitals' });
    
    if (existingPermissions.length > 0) {
      console.log('✅ Hospital permissions already exist:');
    ...
    console.log('🔄 Creating hospital permissions...');
    const createdPermissions = await Permission.insertMany(hospitalPermissions);
    console.log(`✅ Successfully created ${createdPermissions.length} hospital permissions`);
    ...
    if (targetUser) {
      ...
      try {
        ...
        console.log(`✅ Successfully assigned hospital permissions to: ${targetUser.email}`);
      } catch (assignError) {
    ...
    // Summary
    console.log('=====================================');
    console.log('🎉 Hospital Permissions Seeder Completed!');
    console.log('=====================================');
    ...
    console.log('🚀 You can now use Hospital Management features!');
